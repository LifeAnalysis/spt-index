  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 sm:px-6 py-6">
        ...
        {/* Undervalued Protocols Dashboard */}
        {!error && data && data.all.length > 0 && (() => {
          // Calculate undervalued metrics
          const calculateUndervaluedMetrics = (protocols: Protocol[]) => {
            return protocols
              .filter(p => p.tvl > 0 && p.score > 0)
              .map(p => {
                const logTvl = Math.log10(p.tvl);
                
                // Hidden Gem Score: (SPT × Momentum × Growth) / log(TVL)
                const hiddenGemScore = p.momentumScore 
                  ? (p.score * p.momentumScore * (1 + (p.change30d || 0) / 100)) / logTvl
                  : 0;
                
                // Momentum Gap: Shows protocols improving faster than rank suggests
                const momentumGap = p.momentumScore ? p.momentumScore - p.score : 0;
                
                // Basic Value: High score, low TVL
                const basicValue = p.score / logTvl;
                
                // Efficiency metrics (type-specific)
                let efficiencyValue = 0;
                if (p.type === 'dex' && p.dexMetrics) {
                  efficiencyValue = p.dexMetrics.capitalEfficiency * p.score;
                } else if (p.type === 'lending' && p.lendingMetrics) {
                  efficiencyValue = (p.lendingMetrics.utilization / 100) * p.score;
                }
          ...
          const undervaluedProtocols = calculateUndervaluedMetrics(data.all);
          
          // Top Hidden Gems (best overall undervalued)
          const topHiddenGems = [...undervaluedProtocols]
            .sort((a, b) => b.hiddenGemScore - a.hiddenGemScore)
            .slice(0, 5);
          
          // Top Momentum Gap (improving fast)
          const topMomentumGap = [...undervaluedProtocols]
            .filter(p => p.momentumGap > 0)
            .sort((a, b) => b.momentumGap - a.momentumGap)
            .slice(0, 5);
          
          // Top Basic Value (high score, small size)
          const topBasicValue = [...undervaluedProtocols]
            .filter(p => p.tvl < 100000000) // Under $100M TVL
            .sort((a, b) => b.basicValue - a.basicValue)
            .slice(0, 5);
          
          // Top Efficiency
          const topEfficiency = [...undervaluedProtocols]
            .filter(p => p.efficiencyValue > 0)
            .sort((a, b) => b.efficiencyValue - a.efficiencyValue)
            .slice(0, 5);

          const UndervaluedCard = ({ protocol, metric, reason }: { 
            protocol: Protocol & { hiddenGemScore?: number; momentumGap?: number; basicValue?: number; efficiencyValue?: number }, 
            metric: string,
            reason: string 
          }) => {
            const rating = getScoreRating(protocol.score);
            ...
            return (
              <div
                ...
                className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-all cursor-pointer group hover:border-[#49997E]"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="font-bold text-gray-900 truncate group-hover:text-[#49997E] transition-colors">
                        {protocol.protocol}
                      </h4>
                      <span className={`inline-flex px-1.5 py-0.5 rounded text-[10px] font-bold ${rating.color} flex-shrink-0`}>
                        {rating.label}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        ...
                        <div className="font-semibold text-gray-900">{formatCurrency(protocol.tvl)}</div>
                      </div>
                      <div>
                        <div className="text-gray-500">SPT Score</div>
                        <div className="font-semibold text-[#49997E]">{formatScore(protocol.score)}</div>
                      </div>
                      <div>
                        <div className="text-gray-500">30d Change</div>
                        <div className="font-semibold">{formatChange(protocol.change30d)}</div>
                      </div>
                      <div>
                    ...
                    {metric && (
                      <div className="mt-2 pt-2 border-t border-gray-100">
                        ...
                        <div className="font-mono text-xs font-semibold text-blue-600">
                          {protocol.hiddenGemScore && metric.includes('Hidden') && protocol.hiddenGemScore.toFixed(6)}
                          {protocol.momentumGap && metric.includes('Momentum') && `+${protocol.momentumGap.toFixed(4)}`}
                          {protocol.basicValue && metric.includes('Value') && protocol.basicValue.toFixed(4)}
                          {protocol.efficiencyValue && metric.includes('Efficiency') && protocol.efficiencyValue.toFixed(4)}
                        </div>
                      </div>
                    )}
                  </div>
            ...
            <section className="mt-12 mb-12">
              <div className="mb-6">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h3 className="text-xl font-bold text-gray-900">Undervalued Protocols</h3>
                </div>
                <p className="text-sm text-gray-600">
                  High-quality protocols with smaller market caps that may be flying under the radar. 
                  Calculated using composite metrics: SPT Score, Momentum, TVL size, and recent growth.
                </p>
              </div>

              <div className="space-y-6">
                {/* Hidden Gems */}
                {topHiddenGems.length > 0 && (
                  <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl p-4 sm:p-6 border border-purple-200">
                    <div className="flex items-center gap-2 mb-4">
                      <span className="text-xl">🌟</span>
                      <h4 className="font-bold text-gray-900">Top Hidden Gems</h4>
                      <InfoTooltip 
                        content="formula: (spt score × momentum score × (1 + 30d growth)) / log(tvl). finds high-quality protocols with strong momentum but smaller size."
                        position="bottom"
                        maxWidth="500px"
                      />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                      {topHiddenGems.slice(0, 3).map((protocol) => (
                        <UndervaluedCard
                          key={protocol.protocol}
                          protocol={protocol}
                          metric="Hidden Gem Score"
                          reason={`${getScoreRating(protocol.score).label} rating with ${protocol.tvl < 10000000 ? 'only ' : ''}${formatCurrency(protocol.tvl)} TVL`}
                        />
                      ))}
                    </div>
                  </div>
                ...
                {/* Momentum Gap */}
                {topMomentumGap.length > 0 && (
                  <div className="bg-gradient-to-r from-emerald-50 to-green-50 rounded-xl p-4 sm:p-6 border border-emerald-200">
                    <div className="flex items-center gap-2 mb-4">
                      <span className="text-xl">🚀</span>
                      <h4 className="font-bold text-gray-900">Rising Stars</h4>
                      <InfoTooltip 
                        content="formula: momentum score - spt score. positive values mean the protocol is improving faster than its current ranking suggests. early momentum signals."
                        position="bottom"
                        maxWidth="500px"
                      />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                      {topMomentumGap.slice(0, 3).map((protocol) => (
                        <UndervaluedCard
                          key={protocol.protocol}
                          protocol={protocol}
                          metric="Momentum Gap"
                          reason="Improving significantly faster than current rank indicates"
                        />
                      ))}
                    </div>
                  </div>
                ...
                {/* Small Cap Champions */}
                {topBasicValue.length > 0 && (
                  <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl p-4 sm:p-6 border border-amber-200">
                    <div className="flex items-center gap-2 mb-4">
                      <span className="text-xl">🏆</span>
                      <h4 className="font-bold text-gray-900">Small Cap Champions</h4>
                      <InfoTooltip 
                        content="formula: spt score / log(tvl). protocols under $100m tvl with exceptional operational scores. highest quality-to-size ratio."
                        position="bottom"
                        maxWidth="500px"
                      />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                      {topBasicValue.slice(0, 3).map((protocol) => (
                        <UndervaluedCard
                          key={protocol.protocol}
                          protocol={protocol}
                          metric="Value Score"
                          reason={`Exceptional ${protocol.type === 'dex' ? 'trading' : 'lending'} efficiency at small scale`}
                        />
                      ))}
                    </div>
                  </div>
                ...
                {topEfficiency.length > 0 && (
                  <div className="bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl p-4 sm:p-6 border border-blue-200">
                    <div className="flex items-center gap-2 mb-4">
                      <span className="text-xl">⚡</span>
                      <h4 className="font-bold text-gray-900">Efficiency Leaders</h4>
                      <InfoTooltip 
                        content="dex: capital efficiency × spt score (volume/tvl ratio). lending: utilization × spt score (borrowed/supplied ratio). highest operational efficiency."
                        position="bottom"
                        maxWidth="500px"
                      />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                      {topEfficiency.slice(0, 3).map((protocol) => (
                        <UndervaluedCard
                          key={protocol.protocol}
                          protocol={protocol}
                          metric="Efficiency Score"
                          reason={`${protocol.type === 'dex' ? 'Extreme capital efficiency (Volume/TVL)' : 'High utilization rate (Borrowed/Supplied)'}`}
                        />
                      ))}
                    </div>
                  </div>
              ...
              <div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-gray-700">
                <strong>Note:</strong> Undervalued metrics are calculated using composite formulas combining SPT Score, Momentum, TVL, and protocol-specific efficiency metrics. 
                Smaller protocols may show higher volatility. Always conduct your own research.
              </div>
            </section>
          );
        })()}
